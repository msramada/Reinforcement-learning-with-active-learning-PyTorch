from Utilities import FeedForward_NN, ReplayBuffer
import torch


class PPO(object):
    def __init__(self, action_dim, state_dim, hidden_dim, val_lr, actor_lr, action_std, device, 
                 Buffer_size=128*32, gamma=0.95, lambda0=0.95, eps_clip=0.2, max_grad_norm = 0.5, beta=0.01):
        self.Actor = FeedForward_NN(state_dim, hidden_dim, action_dim).to(device)
        self.Value = FeedForward_NN(state_dim, hidden_dim, 1).to(device)
    ...
    def train(self, epochs):
        ...
        horizon_length = time_step_k.max()
        rollouts = (time_step_k == time_step_k.max()).sum()
        Advantages = torch.zeros(horizon_length, rollouts)
        rtgs = torch.zeros(horizon_length, rollouts)
        Advantages[-1,:] = deltas[time_step_k == horizon_length]
        rtgs[-1,:] = rewards[time_step_k == horizon_length]
